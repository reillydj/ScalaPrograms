...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.groupBy((char: Char) => char.toLower)
      .map((pair: (Char, String)) => (pair._1, pair._2.length))
      .toList
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y.toMap foldLeft x.toMap)((leftMap, occurrence) => {
      val (letter, count) = occurrence
      if (leftMap(letter) - count == 0) leftMap - letter
      else leftMap updated (letter, leftMap(letter) - count)
    }).toList.sorted
  }
